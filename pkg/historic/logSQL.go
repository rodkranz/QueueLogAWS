...
func SQLWriter(cfg SQLConfig) (w Writer, err error) {
	...
	if err = x.StoreEngine("InnoDB").Sync2(new(record)); err != nil {
		clog.Warn("sync database struct error: %v\n", err)
		return w, fmt.Errorf("sync database struct error: %v", err)
	}
	x.ShowSQL(true)
	return func(msg *message.Message) error {
		has, err := x.Where("message_id = ?", *msg.MessageId).Get(new(record))
		if err != nil {
			return err
		}
		if has {
...
// record is model of information that will persist at database.
type record struct {
...
func (m *record) BeforeInsert() {
	m.CreatedUnix = time.Now().Unix()
}

// AfterSet after set columns convert unix data to time.Time.
func (m *record) AfterSet(colName string, _ xorm.Cell) {
